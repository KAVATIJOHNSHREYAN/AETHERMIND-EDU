...
export default function Home() {
  ...
  const handleStarterPrompt = async (prompt: string) => {
    if (!token) return;
    try {
      const newChat = await apiService.createChat(token!, prompt.substring(0, 30), activeMode);
  ...
  const handleCreateChat = async (mode: 'general' | 'voice' = 'general') => {
    if (!token) return;
    try {
      const newChat = await apiService.createChat(token!, 'New Conversation', mode === 'general' ? activeMode : mode);
      setChats([newChat, ...chats]);
      setActiveChatId(newChat.id);
      setMessages([]);
  ...
  const handleSendTextMessage = async (e: React.FormEvent) => {
    ...
    if (!currentChatId) {
      try {
        const newChat = await apiService.createChat(token!, chatInput.substring(0, 30), activeMode);
        setChats([newChat, ...chats]);
        setActiveChatId(newChat.id);
        currentChatId = newChat.id;
        setMessages([]);
      ...
      {activeScreen === 'chat' && (
        <div className="flex-1 flex flex-col h-screen overflow-hidden relative z-10">
          ...
          <nav className={`py-3 px-6 border-b backdrop-blur-xl flex items-center justify-between transition-colors z-20 ${
            ...
            {/* Mode Selector */}
            <div className={`hidden md:flex items-center gap-1 p-1 rounded-xl border transition-all ${isDark ? 'bg-slate-900/60 border-slate-800' : 'bg-slate-100 border-slate-200'} backdrop-blur-md overflow-x-auto max-w-lg`}>
              {['general', 'study', 'coding', 'research', 'career', 'interview', 'resume'].map(mode => (
                <button
                  key={mode}
                  onClick={() => setActiveMode(mode)}
                  className={`px-3 py-1.5 rounded-lg text-[10px] font-bold capitalize transition-all whitespace-nowrap ${
                    activeMode === mode 
                      ? (isHacker ? 'bg-emerald-900/50 text-emerald-400 border border-emerald-500/50' : 'bg-violet-600 text-white shadow-md')
                      : (isHacker ? 'text-emerald-700 hover:text-emerald-500' : isDark ? 'text-slate-400 hover:text-slate-200 hover:bg-slate-800' : 'text-slate-500 hover:text-slate-900 hover:bg-slate-200')
                  }`}
                >
                  {mode}
                </button>
